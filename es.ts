...
import Semaphore from "./public/semaphore/Semaphore.js";
...
if (!/^\d+$/.test(ES_PARALLEL_RAW)) {
  throw th(
    `Invalid ES_PARALLEL value: "${ES_PARALLEL_RAW}". It must be a numeric string. Default is 2.`,
  );
}
...
if (PRODUCE_GITIGNORE && gitignorePaths.length > 0) {
  ...
  if (UPDATE_GITIGNORE) {
    ...
    if (startIndex > endIndex) {
      throw th(
        `.gitignore markers are in wrong order: '${startMarker}' appears after '${endMarker}'`,
      );
    }
