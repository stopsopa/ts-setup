export class Vault {
  ...
  public deposit(amount: number): void {
    if (amount <= 0) {
      throw new Error("Vault error: Deposit amount must be positive");
    }
    this.#balance += amount;
  }
  ...
  public withdraw(amount: number, key: string): number {
    ...
    if (amount > this.#balance) {
      throw new Error("Vault error: Insufficient balance");
    }
  ...
  #validateKey(key: string): void {
    if (key !== this.secretKey) {
      throw new Error("Vault error: Invalid secret key");
    }
  }
}
